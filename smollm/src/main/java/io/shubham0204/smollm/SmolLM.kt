...
class SmolLM {
    ...
    /**
     * Clears in-memory conversation state and KV/cache state without unloading model weights.
     * This is intended for batch-style fresh single-turn inference while keeping the model loaded.
     */
    fun resetState() {
        verifyHandle()
        resetState(nativePtr)
    }
    ...
    private external fun resetState(modelPtr: Long)
